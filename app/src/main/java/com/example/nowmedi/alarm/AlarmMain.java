                    ...
                    public void onPositiveButtonClick(Pair<Long, Long> selection) {
                       ...
                       String td = simpleDateFormat.format(today);
                        ...
                        if(date1.before(today)){
                            Toast.makeText(AlarmMain.this, "시작날짜는 오늘이거나 그이후여야 합니다, 오늘날짜"+td , Toast.LENGTH_SHORT).show();
                        }
                        else{
...
//            db.close();
    ...
    public boolean is_name_duplicated(){
        ...
        if(is_empty != null) {
            Toast.makeText(this, mediname+"은 데이터베이스에 있습니다. 사용 불가능", Toast.LENGTH_SHORT).show();
            return true;
        }
        else{
        ...
        for (int i =0; i<arraylist.size();i++ ) {
            ...
            for(int j=0; j<snooze_time;j++){
                ...
                if(duplicated_alarm_name != null) {
                    ...
                    Toast.makeText(this, "선택한 시간은"+Alarm_medi_name.get(Alarm_medi_name.size()-1)+"약 알람시간"+time+"과 겹칠수 있습니다. 다른시간을 선택하세요", Toast.LENGTH_SHORT).show();
                    is_duplicated = true;
                }
                calendar.add(Calendar.MINUTE,1);
            }
